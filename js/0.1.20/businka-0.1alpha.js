...
return uri.replace(/https\:\/\/www\.youtube\.com\/watch\?v=/, "https://www.youtube.com/embed/");
			...
			show: function (opt, src){
			 ...
			 $.extend(true, Businka, opt);
			 $.extend(Businka.image, src);
			$.when(Businka.build()).then(function(){
				$(elements.main).removeClass(Businka.loader.type);
				});
			 if(src){
				 var el =[];
				 var th = [];
				 if (typeof(Businka.image[0]) === 'object'){	 
					 for(var k=0;k<Businka.image.length; k++){
						 	if(/youtu/.test(Businka.image[k].href)){
								var newURI = Businka.convertURI(Businka.image[k].href);
								 Businka.initializeV(newURI, el, k);
								console.log(cache); 
								if(Businka.thumb){
									$(elements.thumb).append(th[k] = $("<div data-index='"+k+"' class= 'bk-thumb video-thumb'>"));
							 		$(th[k]).css("background-image","url("+Businka.getVideoThumb(Businka.image[k].href)+")");
									}
								} 
								else {
							 $(elements.pages).append(el[k] = $("<div class='bk-pages-content'>"));
							 $(el[k]).append("<img src="+Businka.image[k].href+" class='bk-image'>");
							 $(el[k]).append("<div class='bk-image-caption'>"+Businka.image[k].caption+"</div>");
							 if (Businka.thumb){
							 $(elements.thumb).append(th[k] = $("<div data-index='"+k+"' class= 'bk-thumb'>"));
							 $(th[k]).css("background-image","url("+Businka.image[k].href+")");
							 }
								}
						 };
					 };
				 if (typeof(Businka.image[0]) === 'string'){
					 for(var k=0;k<Busina.image.length;k++){
						 $(elements.pages).append(el[k] = $("<div class='bk-pages-content'>"));
						 $(el[k]).append("<img src="+Businka.image[k]+" class='bk-image'/>");
						 if (Businka.thumb){
							 $(elements.thumb).append(th[k] = $("<div class= 'bk-thumb-wrap'>"));
							 $(th[k]).css("background-image","url("+Businka.image[k]+")");
							 };
						 }
					 }
				 };
